...
def compactness_cosine(X, labels):
    """
    Tính độ nén dựa trên khoảng cách Cosine từ các điểm đến tâm cụm.
    -> CÀNG NHỎ CÀNG TỐT (Cluster chặt chẽ)
    
    X      : Document-topic matrix (doc_topic_matrix)
...
def separation_cosine(X, labels):
    """
    Tính độ tách biệt dựa trên khoảng cách Cosine giữa các tâm cụm.
    -> CÀNG LỚN CÀNG TỐT (Các cụm xa nhau)
    """
    centroids = []
    unique_labels = np.unique(labels)
...
class TopicModelManager:
    ...
    def _calculate_metrics_and_caption(self, topic_term_matrix, doc_topic_matrix, model_type):
        ...
        # Sep càng lớn (xa nhau) càng tốt, Comp càng nhỏ (nén chặt) càng tốt
    ...
    def run_bertopic(self):
        # BERTopic có cách tính khác hẳn, tạm thời trả về format cơ bản
        ...
        for i, row in freq.iterrows():
            ...
            topics_data.append({
                "Topic_ID": row['Topic'],
                "Caption": row['Name'],
                "Dominant_Count": row['Count']
            })
        return topics_data, 0, 0, 0